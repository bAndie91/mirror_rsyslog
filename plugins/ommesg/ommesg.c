...
static rsRetVal wallmsg(uchar* pMsg, instanceData *pData)
{
	...
	while((uptr = getutent())) {
		...
		if((ttyf = open(p, O_WRONLY|O_NOCTTY|O_NONBLOCK)) >= 0) {
			if(fstat(ttyf, &statb) == 0 && (statb.st_mode & S_IWRITE) && (statb.st_mode & (S_IWGRP | S_IWOTH))) {
...
CODE_STD_STRING_REQUESTparseSelectorAct(1)
	    ...
	    * plus '*' for wall
	    */
	if(!strncmp((char*) p, ":ommesg:", sizeof(":ommesg:") - 1)) {
		p += sizeof(":ommesg:") - 1; /* eat indicator sequence  (-1 because of '\0'!) */
